...
class options():
	def __init__(self):
		...
		self.dataset_path = os.path.join(gs.data_path, 'DDR/lesion_segmentation')
...
class RetSegDDR():
	...
	def LoadData(self):
		self.ds = DDR(dataset_path=self.opt.dataset_path)
	...
	def ComputeLoss(self):
		...
		self.losses['total'] = (self.losses['style_gram_src']) * 1 + \
		                       (self.losses['cmp_self_src']) * 1e-3 

		#ssim损失
		# if self.opt.ssim > 0:
		# 	self.losses['ssim'] = 1 - self.loss_funcs['ssim'](self.tensors['src_seg'] * self.tensors['src_msk'],
	...
	def Train(self):
		...
		for self.epoch in range(self.epoch_st, self.opt.total_epoch):
			...
			for self.it in range(len(self.ds.src_train)):
				...
				im_train_list = PatchRandMirror([np.copy(self.ds.src_train[self.it]), np.copy(self.ds.src_train_msk[self.it])])
				...
				self.tensors['src_msk'] = torch.cuda.FloatTensor( ndarray2tensor(im_train_list[1].copy()), device=self.cuda_device[0] )
			...
			tnow = time.time()
			print("Epoch:[%d/%d]"%(self.epoch + 1, self.opt.total_epoch), 'Total time:%.02fsec'%(tnow-t0), '\t|| Epoch time:%.02fsec'%(tnow-tpast), end=' -- ram ')
			tpast = tnow
